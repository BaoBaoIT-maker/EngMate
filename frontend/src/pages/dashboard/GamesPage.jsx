...
const games = [
  { id: 'matching', emoji: '🧩', name: 'Nối từ (Matching)', desc: 'Tìm cặp từ tiếng Anh - Việt tương ứng', tag: 'Từ vựng', coming: false },
  { id: 'fill-blank', emoji: '🔤', name: 'Điền từ vào chỗ trống', desc: 'Hoàn thành câu với từ đúng', tag: 'Ngữ pháp', coming: false },
  { emoji: '🔊', name: 'Nghe và chọn', desc: 'Nghe phát âm, chọn từ đúng', tag: 'Phát âm', coming: true },
    ...
    <div className="screen-enter w-full max-w-7xl mx-auto">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem' }}>
        {games.map((g, i) => (
          <div key={i} 
            onClick={() => {
              if (!g.coming && g.id) navigate(`/dashboard/games/${g.id}`);
            }}
            style={{ ...card(t), padding: '1.25rem', cursor: g.coming ? 'default' : 'pointer', opacity: g.coming ? 0.6 : 1, transition: 'transform 0.2s, box-shadow 0.2s', position: 'relative', overflow: 'hidden' }}
